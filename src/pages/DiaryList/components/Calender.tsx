...
const RenderHeader = ({
  ...
  return (
    <DL.HeaderContainer>
      <DL.HeaderYear>{format(currentMonth, 'yyyy')}</DL.HeaderYear>
      <DL.HeaderDate>
...
const RenderCells = ({
  ...
  while (day <= endDate) {
            ...
            <DL.IconWrap>
              <Link to='/write' state={{ date: day }}>
                <div>
                  <DL.PlusCircle
                    className={`${
                      format(cloneDay, 'yyyy-MM-dd') === clickDate ||
                      format(cloneDay, 'yyyy-MM-dd') === hoveredDate
                        ? 'hover-close'
                        : 'hide'
                    }`}
                  />
    ...
    rows.push(<DL.DaysRow key={day}>{days}</DL.DaysRow>);
    days = [];
  }
