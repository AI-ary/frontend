  ...
  return (
    <S.Container>
      <S.InputWrap>
        ...
        />
        <S.WarningWrap>
          <S.WarningContent valid={nicknameValid()}>
            특수문자 혹은 모음, 자음은 사용하실 수 없습니다.
          </S.WarningContent>
        </S.WarningWrap>
        <S.Input
          type='text'
          ...
          isValid={email ? !emailValid() : emailValid()}
        />
        <S.WarningWrap>
          <S.WarningContent valid={email ? !emailValid() : emailValid()}>
            이메일 형식으로 입력해 주세요.
          </S.WarningContent>
        </S.WarningWrap>
        <S.PasswordInputWrap>
          <S.Input
            ...
            isValid={password ? !passwordValid() : passwordValid()}
          />
        ...
        <S.WarningWrap>
          <S.WarningContent
            valid={password ? !passwordValid() : passwordValid()}
          >
            대소문자와 숫자, 특수문자를 포함한 8자 이상 16자 이하의 비밀번호를
            입력해야 합니다.
          </S.WarningContent>
        </S.WarningWrap>
        <S.PasswordInputWrap>
          <S.Input
            ...
            isValid={confirm ? (!confirm ? isSame() : !isSame()) : false}
          />
          <S.ToggleVisibleButton
            onClick={() =>
        ...
        <S.WarningWrap>
          <S.WarningContent
            valid={confirm ? (!confirm ? isSame() : !isSame()) : false}
          >
            비밀번호를 다시 확인해 주세요.
          </S.WarningContent>
        </S.WarningWrap>
      </S.InputWrap>
      <C.CommonFilledBtn disabled={Valid()} isValid={Valid()} onClick={onClick}>
        계정 생성
      </C.CommonFilledBtn>
      {nicknameError && (
        <Modal
