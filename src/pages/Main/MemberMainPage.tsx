...
function AfterLogin() {
  ...
  const startDiary = async () => {
    ...
    if (extension.current) {
      let myImg = selected.replace(
        `data:image/${extension.current};base64,`,
        ''
      );
  ...
  return (
    <ClosedBook>
      ...
      <O.Profile>
        <img
          alt='star'
          src={!!selected ? `${selected}` : 'images/rainbow.png'}
        />
      </O.Profile>
      ...
      <input
        ...
        ref={imgRef}
      />
      <C.CommonFilledBtn onClick={startDiary} isValid={false}>
        시작하기
      </C.CommonFilledBtn>
      <LogoutBtn />
      {success && (
