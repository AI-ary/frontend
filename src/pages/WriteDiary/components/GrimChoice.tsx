...
function GrimChoice() {
  ...
  const NextArrow = (props: { onClick: () => void; isDisabled: boolean }) => {
    ...
    return (
      <DW.Arrow
        className={`right ${isDisabled ? 'disabled' : ''}`}
  ...
  const PrevArrow = (props: { onClick: () => void; isDisabled: boolean }) => {
    ...
    return (
      <DW.Arrow
        className={`left ${isDisabled ? 'disabled' : ''}`}
  ...
  const settings = {
    ...
    nextArrow: (
      <NextArrow
        isDisabled={currentSlide >= keywordList?.length - 4}
  ...
  return (
    <D.DiviContainer>
      <DW.Choicetitle>'AIARY가 분석한 그림이에요!'</DW.Choicetitle>
      <DW.ChoiceWrap>
        {keywordList.length > 0 && (
          <DW.ChoiceKeyword>
            <DW.StyledSlider {...settings}>
              {keywordList &&
                keywordList.map((x, index) => (
                  <DW.Keyword
                    isSelected={selectedKeyword === x}
                    key={index}
                    onClick={() => setSelectedKeyword(x)}
                  >
                    <p>{x}</p>
                    {x}
                  </DW.Keyword>
