...
import {
  addKonlpyTextData,
  sendKonlpyPollingData,
  getKonlpyDrawingData,
} from '@/apis/konlpy';
import {
  addDalleTextData,
...
function DiaryContent(props: DiaryContentProps) {
  ...
  const [modalContent, setModalContent] =
    useState<string>('달리에서 이미지를 가져올까요?');
  ...
  const { isKonlpyPollingSuccess, konlpyState, sendKonlpyPollingState } =
    sendKonlpyPollingData();
  const {
  ...
  const resetImgList = (isKonlply: number) => {
    ...
    setModalTitle(
      isKonlply === 2
        ? '달리를 가져오게 되면 기존 그림이 사라집니다.'
        : '그림을 가져오게 되면 달리 그림이 사라집니다.'
    );
    setModalContent(
      isKonlply === 2 ? '달리에서 이미지를 가져올까요?' : '그림을 가져올까요?'
    );
    if (isKonlply === 2) {
          ...
          <DW.ButtonContainer>
            {!grim ? (
              <>
                ...
                <DW.Modebutton onClick={bringGrim}>그림가져오기</DW.Modebutton>
                {btnType !== 2 ? (
                  <DW.Modebutton
                    onClick={() => {
                      setGrim(true);
