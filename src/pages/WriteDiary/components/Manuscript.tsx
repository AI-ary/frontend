...
function Manuscript(props: any) {
  ...
  const showManuscript = emptySections.map(
    (emptySection: unknown, idx: number) => (
  ...
  return (
    <D.PaperContainer>
      <DW.GridContent
        ...
        value={word}
        onChange={wordInput}
        onKeyUp={(e) => moveCursor(e, moveCursorDatas)}
      />
      <label htmlFor='word'>
        <DW.ShowManuScriptWrap>{showManuscript}</DW.ShowManuScriptWrap>
      </label>
    </D.PaperContainer>
  );
}
