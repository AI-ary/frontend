...
const moveCursor = (e: React.KeyboardEvent, props: moveCursorProps) => {
  if (e.code === 'ArrowUp') {
    ...
    if (props.cursor - 10 >= 0) {
      props.cursorLocation.selectionStart = props.cursor - 10;
    } else {
      props.cursorLocation.selectionStart = 0;
    }
  } else if (e.code === 'ArrowDown') {
    // 화살표 아래를 누를 시 10칸 올라가기 때문에 +10을 한다.
    // 만약 업데이트 전 값이 현재 글의 길이보다 크다면 맨 뒤로 위치시킨다.
    props.setCursor((prev: number) =>
      prev + 10 > props.word.length ? props.word.length : prev + 10
    );
    ...
    if (props.cursor + 10 < props.word.length) {
      ...
      props.cursorLocation.selectionStart = props.word.length;
    }
  } else if (e.code === 'ArrowRight') {
    // 화살표 오른쪽을 누를 시 1칸 옮기기 때문에 +1을 한다.
    // 만약 업데이트 전 값이 현재 글의 길이보다 크다면 맨 뒤로 위치시킨다.
    props.setCursor((prev: number) =>
      prev + 1 > props.word.length ? props.word.length : prev + 1
    );

    // textarea의 cursor 위치 또한 조정
    if (props.cursor + 1 <= props.word.length) {
      props.cursorLocation.selectionStart = props.cursor + 1;
    } else {
      props.cursorLocation.selectionStart = props.word.length;
    }
    ...
    props.setCursor(props.cursorLocation.selectionStart);
  }
};
