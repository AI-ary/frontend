...
export const useStore = create<StoreState & StoreActions>((set) => ({
  ...
  loading: false, // 로딩창
  setCurrentCanvas: (updateCanvas: string) =>
    set({ currentCanvas: updateCanvas }),
  setUpdateCanvas: (canvas: string) => set({ updateCanvas: canvas }),
  setGetGrimList: (data: any[string]) => set({ getGrimList: data }),
  setGetDalleList: (data: string[]) =>
    set((state: StoreState & StoreActions) => ({
      ...state,
      getDalleList: data,
    })),
  setChoiceImg: (
  ...
  setBringMoreDalleWarning: (state: boolean) =>
    set({ bringMoreDalleWarning: state }),
