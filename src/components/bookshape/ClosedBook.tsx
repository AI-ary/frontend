...
const ClosedBook = ({ children }: React.PropsWithChildren) => {
  ...
  return (
    <S.Container className='slide'>
      <S.ThemeContainer>
        {!themeMenu ? (
          <S.StyledHiddenPalette onClick={() => setThemeMenu(true)} />
        ) : currentTheme ? (
          <S.StyledCheckIcon onClick={changeTheme} />
        ) : (
          <S.StyledShowPalette onClick={() => setThemeMenu(false)} />
        )}
